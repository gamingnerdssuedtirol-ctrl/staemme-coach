...
function normalizedRecommendationScore(candidate, ranked) {
  ...
  return clampPercent(45 + ((candidate.score - min) / (max - min)) * 50);
}

function hoursUntilResource(required, current, rate) {
  const missing = required - current;
  if (missing <= 0) return 0;
  return rate > 0 ? missing / rate : Infinity;
}

function resourceForecast(hours) {
  const resources = state.resources || {};
  const production = state.production || {};
  const storage = resources.storage || Infinity;

  const raw = {
    wood: (resources.wood || 0) + (production.woodPerHour || 0) * hours,
    clay: (resources.clay || 0) + (production.clayPerHour || 0) * hours,
    iron: (resources.iron || 0) + (production.ironPerHour || 0) * hours
  };

  return {
    raw,
    stored: {
      wood: Math.min(storage, raw.wood),
      clay: Math.min(storage, raw.clay),
      iron: Math.min(storage, raw.iron)
    },
    overflow: {
      wood: Math.max(0, raw.wood - storage),
      clay: Math.max(0, raw.clay - storage),
      iron: Math.max(0, raw.iron - storage)
    }
  };
}

function storageFillHours(resource, production, storage) {
  if (!Number.isFinite(storage) || production <= 0) return Infinity;
  if (resource >= storage) return 0;
  return (storage - resource) / production;
}
...
function coachRisks() {
  ...
  const freePop = Number.isFinite(pop.max) && Number.isFinite(pop.current)
    ? pop.max - pop.current : null;

  const fill = [
    ["Holz", storageFillHours(r.wood || 0, p.woodPerHour || 0, r.storage)],
    ["Lehm", storageFillHours(r.clay || 0, p.clayPerHour || 0, r.storage)],
    ["Eisen", storageFillHours(r.iron || 0, p.ironPerHour || 0, r.storage)]
  ].sort((a, b) => a[1] - b[1]);

  if (Number.isFinite(fill[0][1]) && fill[0][1] <= 8) {
    risks.push({
      level: fill[0][1] <= 3 ? "high" : "medium",
      text: `${fill[0][0]} erreicht den Speicher in etwa ${durationText(fill[0][1] * 3600)}.`
    });
  }

  if (freePop !== null && freePop < 150) {
    risks.push({
      level: freePop < 75 ? "high" : "medium",
      text: `Nur noch ${fmt(freePop)} freie Bevölkerung. Bauernhof bald priorisieren.`
    });
  }

  const queue = queueSeconds();
  if (queue <= 0) {
    risks.push({level: "high", text: "Die Bauschleife ist leer."});
  } else if (queue < 3600) {
    risks.push({level: "medium", text: "Die Bauschleife endet in weniger als einer Stunde."});
  }

  const prod = [p.woodPerHour || 0, p.clayPerHour || 0, p.ironPerHour || 0];
  const max = Math.max(...prod);
  const min = Math.min(...prod);
  if (max > 0 && min / max < 0.78) {
    const labels = ["Holz", "Lehm", "Eisen"];
    risks.push({
      level: "medium",
      text: `${labels[prod.indexOf(min)]} produziert deutlich weniger als dein stärkster Rohstoff.`
    });
  }
...
function strategicVerdict() {
  ...
  const top = ranked[0];
  const troopPlan = buildTroopPlan();
  const phase = detectGamePhase(effectiveLevels());
  const score = normalizedRecommendationScore(top, ranked);
  const risks = coachRisks();

  let focus = "Ausgewogener Ausbau";
  if (top?.name === "Speicher" || top?.name === "Bauernhof") focus = "Engpass beseitigen";
  else if (BUILDING_DATA[top?.name]?.role === "eco") focus = "Wirtschaft stärken";
  else if (BUILDING_DATA[top?.name]?.role === "military") focus = "Militärproduktion ausbauen";
  else if (["Schmiede", "Marktplatz", "Adelshof"].includes(top?.name)) focus = "Adelshof vorbereiten";

  const troop = troopPlan?.recommendations?.[0] || null;
  ...
  return {
    ...
    risks,
    explanation: top
      ? `${top.name} ${top.target} hat aktuell den größten strategischen Nutzen, weil ${reasonText(top)}.`
      : "Noch keine belastbare Empfehlung verfügbar."
  };
}

const GOAL_DEFINITIONS = {
  snob: {
    label: "Adelshof freischalten",
    requirements: {
      "Hauptgebäude": 20,
      "Schmiede": 20,
      "Marktplatz": 10,
  ...
  light100: {label: "100 Leichte Kavallerie", unit: "light", target: 100},
  light500: {label: "500 Leichte Kavallerie", unit: "light", target: 500},
  axe500: {label: "500 Axtkämpfer", unit: "axe", target: 500},
  axe1000: {label: "1.000 Axtkämpfer", unit: "axe", target: 1000},
  eco25: {
    label: "Alle Rohstoffgebäude auf 25",
    requirements: {"Holzfäller": 25, "Lehmgrube": 25, "Eisenmine": 25}
  }
};

function estimateBuildingGoal(goal) {
  const levels = effectiveLevels();
  const missing = [];
  let estimatedHours = queueSeconds() / 3600;

  for (const [name, target] of Object.entries(goal.requirements || {})) {
    const current = levels[name] || 0;
    if (current >= target) continue;
    missing.push({name, current, target, levels: target - current});

    for (let level = current + 1; level <= target; level++) {
      const duration = simulatedBuildHours(name, level, {...levels, [name]: level - 1});
      estimatedHours += duration.hours;
    }
  }

  return {missing, estimatedHours, achieved: missing.length === 0};
}

function estimateUnitGoal(goal) {
  const units = state.units || {};
  const levels = effectiveLevels();
  const unit = UNIT_DATA[goal.unit];
  const current = Number.isFinite(units[goal.unit]) ? units[goal.unit] : 0;
  const missing = Math.max(0, goal.target - current);
  const buildingLevel = levels[unit.building] || 0;

  if (missing === 0) return {achieved: true, current, missing: 0, estimatedHours: 0};
  if (buildingLevel <= 0) {
    return {achieved: false, current, missing, estimatedHours: Infinity, blocker: `${unit.building} fehlt.`};
  }

  const trainingHours = unitTrainingSeconds(goal.unit, buildingLevel) * missing / 3600;
  const cost = {
    wood: unit.cost.wood * missing,
    clay: unit.cost.clay * missing,
    iron: unit.cost.iron * missing
  };

  const r = state.resources || {};
  const p = state.production || {};
  const resourceHours = Math.max(
    hoursUntilResource(cost.wood, r.wood || 0, p.woodPerHour || 0),
    hoursUntilResource(cost.clay, r.clay || 0, p.clayPerHour || 0),
    hoursUntilResource(cost.iron, r.iron || 0, p.ironPerHour || 0)
  );
  ...
  return {
    ...
    trainingHours,
    resourceHours,
    estimatedHours: Math.max(trainingHours, resourceHours)
  };
}

function goalAnalysis() {
  const goal = GOAL_DEFINITIONS[settings.coachGoal] || GOAL_DEFINITIONS.snob;
  const result = goal.unit ? estimateUnitGoal(goal) : estimateBuildingGoal(goal);
  return {goal, ...result};
}

function formatGoalTime(hours) {
  if (hours === Infinity) return "derzeit blockiert";
  if (!(hours > 0)) return "bereits erreicht";
  if (hours < 24) return `ca. ${durationText(hours * 3600)}`;
  const days = Math.floor(hours / 24);
  const remaining = hours - days * 24;
  return `ca. ${days} T. ${Math.round(remaining)} Std.`;
}

function renderCoachAnalysis() {
  const verdict = strategicVerdict();
  const goal = goalAnalysis();
  const forecastHours = +settings.forecastHours || 24;
  const forecast = resourceForecast(forecastHours);

  $("coachScore").textContent = `${verdict.score}/100`;
  $("coachScoreBar").style.width = `${verdict.score}%`;
  $("coachFocus").textContent = verdict.focus;
  $("coachExplanation").textContent = verdict.explanation;
  $("coachPhase").textContent = verdict.phase.label;

  $("coachBuild").textContent = verdict.top
    ? `${verdict.top.name} ${verdict.top.target}`
    : "–";
  $("coachTroop").textContent = verdict.troop
    ? `${verdict.troop.count} ${verdict.troop.short}`
    : "Keine Ausbildung empfohlen";

  const risks = $("coachRisks");
  risks.innerHTML = "";
  if (!verdict.risks.length) {
    risks.innerHTML = '<div class="coach-risk low">✓ Keine akuten Engpässe erkannt.</div>';
  } else {
    verdict.risks.forEach(risk => {
      const row = document.createElement("div");
      row.className = `coach-risk ${risk.level}`;
      row.textContent = risk.text;
      risks.appendChild(row);
    });
  }

  $("forecastTitle").textContent = `Prognose in ${forecastHours} Stunden`;
  $("forecastWood").textContent = fmt(forecast.stored.wood);
  $("forecastClay").textContent = fmt(forecast.stored.clay);
  $("forecastIron").textContent = fmt(forecast.stored.iron);
  const overflow = forecast.overflow.wood + forecast.overflow.clay + forecast.overflow.iron;
  $("forecastOverflow").textContent = overflow > 0
    ? `${fmt(overflow)} Rohstoffe würden überlaufen`
    : "Kein Überlauf erwartet";

  $("goalName").textContent = goal.goal.label;
  $("goalEta").textContent = formatGoalTime(goal.estimatedHours);

  const details = $("goalDetails");
  if (goal.achieved) {
    details.textContent = "Ziel bereits erreicht.";
  } else if (goal.blocker) {
    details.textContent = goal.blocker;
  } else if (goal.missing && Array.isArray(goal.missing)) {
    details.textContent = goal.missing
      .map(item => `${item.name}: ${item.current} → ${item.target}`)
      .join(" · ");
  } else {
    details.textContent = `Aktuell ${fmt(goal.current)}, noch ${fmt(goal.missing)} benötigt.`;
  }

  $("coachDebug").textContent = JSON.stringify({
    verdict,
    forecastHours,
    forecast,
    goal
...
function render() {
  ...
  $("forecastHours").value = settings.forecastHours;
  $("forecastHoursValue").textContent = `${settings.forecastHours} Std.`;
  $("coachGoal").value = settings.coachGoal;
  renderStatus();
  renderPlanner();
  renderStrategyPlan();
  renderLiveDashboard();
...
$("forecastHours").oninput = (e) => {
  settings.forecastHours = +e.target.value; saveSettings(); render();
};
$("coachGoal").onchange = (e) => {
  settings.coachGoal = e.target.value; saveSettings(); render();
};
...
// v1.2.1 cache repair: remove obsolete app caches after the new build is active.
(async function repairOldCaches() {
  try {
    if ("caches" in window) {
      const keys = await caches.keys();
      await Promise.all(
        keys
          .filter(key => key !== "staemme-coach-v20-clean")
